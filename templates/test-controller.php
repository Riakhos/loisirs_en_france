    ...
    public function chooseDateTime(Request $request): Response
    {
        ...
        if ($orderForm->isSubmitted() && $orderForm->isValid()) {
            ...
            $order
                ...
                ->setActivityName(implode(', ', array_map(function($product) {
                    return isset($product['type']) && $product['type'] === 'activity' ? $product['name'] : null;
                }, $orderData['activities'])))
                ->setOfferName(implode(', ', array_map(function($product) {
                    return isset($product['type']) === 'offer' ? $product['name'] : null;
                }, $orderData['offers'])))
                ->setEventName(implode(', ', array_map(function($product) {
                    return isset($product['type']) === 'event' ? $product['name'] : null;
                }, $orderData['events'])))
                ->setPartnerName(implode(', ', array_map(function($product) {
                    $partner = $this->em->getRepository(Partner::class)->find($product['object']->getPartners()->getId());
